...
abstract type DGStdRegion{QT,ND,NP,NE} <: AbstractStdRegion{ND,NP,NE} end

quadrature(::DGStdRegion{QT}) where {QT} = QT

#==========================================================================================#
#                                     Temporary cache                                      #

struct DGStdRegionCache{S,T,B,SH,SC}
...
function DGStdRegionCache{RT}(nd, np, ::Val{NV}) where {RT,NV}
    ...
    return DGStdRegionCache(tmps, tmpst, tmpb, tmp♯, tmpsubcell)
end

#==========================================================================================#
#                                      Standard point                                      #

struct StdPoint <: DGStdRegion{GaussQuadrature,0,1,1} end
...
struct StdSegment{QT,NP,NE,RT,MM,F,C} <: DGStdRegion{QT,1,NP,NE}
    face::F
...
function StdSegment{RT}(np::Integer, qtype, nvars=1; nequispaced=np) where {RT}
    return StdSegment{RT}(Val(np), qtype, Val(nvars); nequispaced=Val(nequispaced))
end
...
function StdSegment{RT}(
    ...
    # Temporary storage
    cache = DGStdRegionCache{RT}(1, NP, nvars)
...
function massmatrix(std::StdSegment, jac)
    return Diagonal(jac .* diag(std.M))
end
...
struct StdQuad{QT,NP,NE,RT,MM,F,C} <: DGStdRegion{QT,2,NP,NE}
    face::F
...
function StdQuad{RT}(np::Integer, qtype, nvars=1; nequispaced=np) where {RT}
    return StdQuad{RT}(Val(np), qtype, Val(nvars); nequispaced=Val(nequispaced))
end
...
function StdQuad{RT}(
    ...
    # Quadratures
    fstd = StdSegment{RT}(Val(NP), qtype, nvars; nequispaced=nequispaced)
    ...
    # Temporary storage
    cache = DGStdRegionCache{RT}(2, NP, nvars)
...
function massmatrix(std::StdQuad, jac)
    return Diagonal(jac .* diag(std.M))
end
...
struct StdTri{Q,NP,NE} <: DGStdRegion{Q,2,NP,NE} end
...
vtk_type(::StdTri) = UInt8(69)

function vtk_connectivities(::StdTri)
    error("Not implemented yet!")
end
...
struct StdHex{QT,NP,NE,RT,MM,F,E,C} <: DGStdRegion{QT,3,NP,NE}
    face::F
    edge::E
...
function StdHex{RT}(np::Integer, qtype, nvars=1; nequispaced=np) where {RT}
    return StdHex{RT}(Val(np), qtype, Val(nvars); nequispaced=Val(nequispaced))
end
...
function StdHex{RT}(
    ...
    # Quadratures
    fstd = StdQuad{RT}(Val(NP), qtype, nvars; nequispaced=nequispaced)
    estd = fstd.face
    ξ = vec([
    ...
    # Temporary storage
    cache = DGStdRegionCache{RT}(3, NP, nvars)
...
function massmatrix(std::StdHex, jac)
    return Diagonal(jac .* diag(std.M))
end
